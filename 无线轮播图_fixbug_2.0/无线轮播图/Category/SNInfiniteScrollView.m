...
- (void)updateContent {
    ...
    for (int i = 0; i < self.scrollView.subviews.count; i++) {
        ...
        if (pageIndex < 0) {
            pageIndex = self.imageURLs.count - 1;
        ...
        [imageView sd_setImageWithURL:[NSURL URLWithString:self.imageURLs[imageView.tag]] placeholderImage:nil];
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (int i = 0; i < self.scrollView.subviews.count; i++) {
        ...
        if (self.scrollDirection == ScrollDirectionVertical) {
            // fix bug , 过中线 page改变.
            if (ABS(point.y - self.frame.origin.y) < scrollViewHeight * 0.5) {
                page = imageView.tag;
            }
        } else {
            // fix bug , 过中线 page改变.
            if (ABS(point.x - self.frame.origin.x) < scrollViewWidth * 0.5) {
                page = imageView.tag;
            }
        }
    }
...
- (void)startTimer {
    ...
    NSTimer * timer = [NSTimer scheduledTimerWithTimeInterval:2.0 target:self selector:@selector(next:) userInfo:nil repeats:YES];
